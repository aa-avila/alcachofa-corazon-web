console.log('starting catalog.platos.js');
...
const { CATALOG_IMG_PLATOS, CATALOG_PDF_PLATOS } = URLS;
...
const createCarousel = async () => {
  const htmlCarousel = await carousel(`${imgHost}${CATALOG_IMG_PLATOS}`);
  renderById('carousel-catalog', htmlCarousel);
};

const setupDwnPdfButton = async () => {
  pdfDownloadBtn.href = CATALOG_PDF_PLATOS;
...
const setupAnalytics = async () => {
  ...
  logEvent(analytics, events.CATALOG_PLATOS_VIEW, { timestamp: new Date() });

  pdfDownloadBtn.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_PLATOS_BTN_DOWNLOAD_CLICKED, {
      timestamp: new Date()
    });
  ...
  wpFloatBtn.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_PLATOS_BTN_WP_CLICKED, {
      timestamp: new Date()
    });
  ...
  backBtnTop.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_PLATOS_BTN_BACK_TOP_CLICKED, {
      timestamp: new Date()
    });
  });

  navbarBrand[0].addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_PLATOS_NAVBRAND_CLICKED, {
      timestamp: new Date()
    });
  });

  homeNavbarLink.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_PLATOS_NAVLINK_HOME_CLICKED, {
      timestamp: new Date()
    });
  });

  wpNavbarLink.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_PLATOS_NAVLINK_WP_CLICKED, {
      timestamp: new Date()
    });
    logEvent(analytics, events.GO_TO_WHATSAPP, {
      timestamp: new Date()
