console.log('starting catalog.macetas.js');
...
const setupDwnPdfButton = async () => {
  pdfDownloadBtn.href = CATALOG_PDF_MACETAS;
...
const setupAnalytics = async () => {
  ...
  logEvent(analytics, events.CATALOG_MACETAS_VIEW, { timestamp: new Date() });

  pdfDownloadBtn.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_MACETAS_BTN_DOWNLOAD_CLICKED, {
      timestamp: new Date()
    });
  ...
  wpFloatBtn.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_MACETAS_BTN_WP_CLICKED, {
      timestamp: new Date()
    });
  ...
  backBtnTop.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_MACETAS_BTN_BACK_TOP_CLICKED, {
      timestamp: new Date()
    });
  });

  navbarBrand[0].addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_MACETAS_NAVBRAND_CLICKED, {
      timestamp: new Date()
    });
  });

  homeNavbarLink.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_MACETAS_NAVLINK_HOME_CLICKED, {
      timestamp: new Date()
    });
  });

  wpNavbarLink.addEventListener('click', () => {
    logEvent(analytics, events.CATALOG_MACETAS_NAVLINK_WP_CLICKED, {
      timestamp: new Date()
    });
    logEvent(analytics, events.GO_TO_WHATSAPP, {
      timestamp: new Date()
